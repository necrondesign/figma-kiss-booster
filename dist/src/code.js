...
async function findDarkMode() {
    ...
    const queue = [...figma.currentPage.selection];
    let checked = 0;
                ...
                for (const val of Object.values(bv)) {
                    ...
                    for (const b of bindings) {
                        ...
                        try {
                            ...
                            if (!v || checkedIds.has(v.variableCollectionId))
                                continue;
                            ...
                            if (col) {
                                ...
                                if (dark)
                                    return { collection: col, modeId: dark.modeId };
                            }
                        }
                        catch (_e) { }
                    }
...
async function findVariable(name) {
    ...
    const search = (vars) => {
        ...
        return partial !== null && partial !== void 0 ? partial : null;
    };
...
function detectOrientation(frames) {
    if (frames.length < 2)
        return "horizontal";
...
function placeDarkCopy(dark, light, orientation) {
...
async function darkThemeCopy() {
    ...
    const sectionSelected = selection.find((n) => n.type === "SECTION");
    let lightFrames = [];
    let container = null;
    if (sectionSelected) {
    ...
    if (sectionSelected && clones.length > 0) {
        ...
        sectionSelected.resizeWithoutConstraints(sectionSelected.width, maxY + SECTION_PADDING);
    }
    if (clones.length > 0) {
...
async function wrapToNewSelection(withDark = true) {
    const selection = [...figma.currentPage.selection];
    if (selection.length === 0) {
        sendStatus("Select frames", "error");
        return;
    ...
    if (dark) {
        ...
        const existingDarkByLightName = new Map();
        ...
        const newDarkFrames = [];
        for (const frame of lightFrames) {
            if (existingDarkByLightName.has(frame.name))
                continue;
            if (!("clone" in frame))
                continue;
    ...
    section.resizeWithoutConstraints(contentW + SECTION_PADDING * 2, contentH + SECTION_PADDING * 2);
    // Apply fill token
    const fillVar = await findVariable("default_system_frame");
    if (fillVar) {
        try {
            const baseFill = { type: "SOLID", color: { r: 1, g: 1, b: 1 } };
...
async function alignSections() {
    ...
    let sections = [];
    if (selection.length > 0) {
        sections = selection.filter((n) => n.type === "SECTION");
    }
...
async function toggleDevStatus() {
    const selection = [...figma.currentPage.selection];
    let targets = [];
    if (selection.length > 0) {
    ...
    const allReady = targets.every((n) => { var _a; return ((_a = n.devStatus) === null || _a === void 0 ? void 0 : _a.type) === "READY_FOR_DEV"; });
    for (const node of targets) {
        node.devStatus = allReady
            ? null
            : { type: "READY_FOR_DEV" };
    }
}
...
function shiftRowSectionsRight(section, fromX, by) {
    for (const s of figma.currentPage.children) {
        if (s.type !== "SECTION" || s.id === section.id)
            continue;
        const sec = s;
        if (sec.x >= fromX &&
...
async function expandSectionGrow(direction, duplicate = true) {
    const selection = [...figma.currentPage.selection];
    ...
    if (FRAMEISH.has(node.type)) {
        ...
        for (const c of section.children) {
            if (!FRAMEISH.has(c.type))
                continue;
            if (direction === "right") {
                if (c.id !== frame.id && c.x > originalFrameX)
                    c.x += expandBy;
            }
            else {
                if (c.x >= originalFrameX)
                    c.x += expandBy; // includes the frame itself
            }
        }
        if (duplicate) {
            const clone = frame.clone();
...
async function replaceWithInstance() {
    ...
    const source = (lastAddedId && selection.find((n) => n.id === lastAddedId)) ||
    ...
    for (const target of targets) {
        ...
        if (constraints && "constraints" in clone)
...
async function findSimilar() {
    ...
    const matches = candidates.filter((n) => n.name === name && Math.round(n.width) === w && Math.round(n.height) === h);
    if (matches.length <= 1) {
...
async function fixSelection(withDark = true) {
    const selection = [...figma.currentPage.selection];
...
let cachedArtTask = null;
async function findArtTaskComponent() {
    if (cachedArtTask && !cachedArtTask.removed)
...
async function fillArtTask(node, sizeText, size3xText) {
    const iconTags = findIconTags(node);
    if (iconTags.length < 3)
        return;
    const tag1Texts = findTextNodes(iconTags[1]);
    ...
    if (tag1Texts.length > 0)
        tag1Texts[0].characters = sizeText;
    if (tag2Texts.length > 0)
        tag2Texts[tag2Texts.length - 1].characters = size3xText;
}
async function createArtBlock() {
    const selection = [...figma.currentPage.selection];
    if (selection.length !== 2) {
...
async function frameWithBorder() {
    const selection = [...figma.currentPage.selection];
    if (selection.length === 0) {
    ...
    for (const node of selection) {
        ...
        const parentAbsX = "absoluteTransform" in parent ? parent.absoluteTransform[0][2] : 0;
        const parentAbsY = "absoluteTransform" in parent ? parent.absoluteTransform[1][2] : 0;
...
function findAllTextNodes(nodes) {
    ...
    for (const node of nodes) {
        if (node.type === "TEXT") {
            ...
            result = result.concat(findAllTextNodes(node.children));
        }
    }
    return result;
}
                ...
                for (const item of results) {
                    ...
                    if (node && node.type === "TEXT") {
                        ...
                        if (fontToLoad === figma.mixed) {
                            fontToLoad = textNode.getRangeFontName(0, 1);
