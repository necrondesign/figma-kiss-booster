...
async function wrapToNewSelection() {
    const selection = [...figma.currentPage.selection];
    if (selection.length === 0) {
        sendStatus("Select frames", "error");
        return;
...
async function findArtTaskComponent() {
    ...
    for (const page of pages) {
        const search = page.findOne((n) => {
            ...
            if (n.type === "INSTANCE" && name.includes("arttask"))
                return true;
            if (n.type === "COMPONENT" && name.includes("arttask"))
                return true;
            return false;
        });
        if (search) {
            ...
            if (search.type === "INSTANCE") {
                const main = search.mainComponent;
                if (main)
                    return main;
            }
        }
...
async function findDesignStickerComponent() {
    const pages = figma.root.children;
    for (const page of pages) {
        const search = page.findOne((n) => {
            const name = n.name.toLowerCase().replace(/\s+/g, "");
            if (n.type === "INSTANCE" && name.includes("designsticker"))
                return true;
            if (n.type === "COMPONENT" && name.includes("designsticker"))
                return true;
            return false;
        });
        if (search) {
...
async function createDesignSticker(customName) {
    const selection = [...figma.currentPage.selection];
    if (selection.length === 0) {
    ...
    let stickerComponent = found.type === "COMPONENT_SET"
        ? found.children[0]
        : found;
    ...
    if (targetParent && "children" in targetParent) {
        ...
        targetParent.insertChild(idx + 1, instance);
        instance.x = bb ? bb.x - targetParent.absoluteTransform[0][2] : instance.x;
        instance.y = bb ? bb.y - instance.height + 40 - targetParent.absoluteTransform[1][2] : instance.y;
    }
...
figma.ui.onmessage = async (msg) => {
    switch (msg.type) {
        ...
        case "frame-border":
            ...
            break;
        case "create-sticker":
            await createDesignSticker(msg.name || "");
            break;
